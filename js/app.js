...
(function () {
  ...
  // ----- استخراج كشف الحساب من ملف PDF -----
  // يعتمد على pdf.js لاستخراج النص، ثم تحليل كل سطر بحثًا عن
  // نمط (تاريخ ... مبلغ) شائع في كشوفات الحسابات. يعمل فقط مع
  // ملفات PDF نصّية (وليست صورًا ممسوحة ضوئيًا بلا طبقة نص).
  async function readPdfFile(file) {
    if (typeof pdfjsLib === "undefined") {
      throw new Error("تعذّر تحميل مكتبة قراءة PDF — تحقق من الاتصال بالإنترنت وأعد المحاولة");
    }
    ...
    if (!rows.length) {
      throw new Error("لم يتم العثور على حركات مالية واضحة داخل الـ PDF. إن كان الملف صورة ممسوحة ضوئيًا (بلا طبقة نص) فهذا غير مدعوم — حوّله إلى Excel/CSV أو استخدم أداة OCR أولًا");
    }
    return { headers: ["التاريخ", "البيان", "المبلغ"], rows };
  }

  function groupTextItemsIntoLines(items) {
    // كل عنصر نصي له إحداثيات (x, y) — نجمّع العناصر المتقاربة بنفس الارتفاع (y) في سطر واحد
    const buckets = [];
    items.forEach(it => {
      const y = Math.round(it.transform[5]);
      let bucket = buckets.find(b => Math.abs(b.y - y) <= 2);
      if (!bucket) { bucket = { y, parts: [] }; buckets.push(bucket); }
      bucket.parts.push({ x: it.transform[4], text: it.str });
    });
    // ترتيب الأسطر من الأعلى للأسفل بحسب الصفحة، وداخل كل سطر بحسب الموضع الأفقي
    buckets.sort((a, b) => b.y - a.y);
    return buckets
      .map(b => b.parts.sort((p1, p2) => p1.x - p2.x).map(p => p.text).join(" ").replace(/\s+/g, " ").trim())
      .filter(Boolean);
  }

  const PDF_DATE_RE = /^\s*(\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}|\d{4}[\/\-.]\d{1,2}[\/\-.]\d{1,2})/;
  const PDF_AMOUNT_RE = /\(?-?\d[\d,]*\.\d{2}\)?/g;
  ...
  function parseStatementLines(lines) {
    ...
    lines.forEach(rawLine => {
      ...
      if (!dateMatch) return; // سطر بلا تاريخ في بدايته — على الأغلب عنوان/تذييل، تجاهله

      const dateStr = dateMatch[0].trim();
      const rest = line.slice(dateMatch[0].length);

      const amounts = rest.match(PDF_AMOUNT_RE);
      if (!amounts || !amounts.length) return; // سطر بلا مبلغ — تجاهله

      const amountStr = amounts[0]; // أول رقم مالي بعد التاريخ = مبلغ الحركة (الأرقام التالية غالبًا رصيد متراكم)
      let desc = rest;
      amounts.forEach(a => { desc = desc.replace(a, " "); });
      desc = desc.replace(/\s+/g, " ").trim().replace(/^[-–,:\s]+|[-–,:\s]+$/g, "");

      rows.push({ "التاريخ": dateStr, "البيان": desc, "المبلغ": amountStr });
    });
    return rows;
  }
